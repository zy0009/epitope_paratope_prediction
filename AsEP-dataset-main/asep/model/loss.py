...
def edge_index_bg_rec_loss_balanced(
    edge_index_bg_pred: Tensor,
    edge_index_bg_true: Tensor,
    ...
    """
    对一个 Ab-Ag 图对计算二分类 loss，正负分开算再加权。
    这样正样本不会被一堆 0 淹没。
    """
    # 展平
    y_pred = edge_index_bg_pred.reshape(-1)
    y_true = edge_index_bg_true.reshape(-1)
    ...
    if pos_mask.any():
        pos_loss = F.binary_cross_entropy(
            y_pred[pos_mask], y_true[pos_mask].float(), reduction="mean"
        )
    else:
        pos_loss = y_pred.new_tensor(0.0)

    if neg_mask.any():
        neg_loss = F.binary_cross_entropy(
            y_pred[neg_mask], y_true[neg_mask].float(), reduction="mean"
        )
    else:
        neg_loss = y_pred.new_tensor(0.0)
